...
namespace Csharp_Project
{
    public partial class FORM_MANAGE_ORDER : Form
    {
        ...
        public FORM_MANAGE_ORDER()
        {
            ...
            COMBO_CATEGORIES.DisplayMember = "Nom_CAT";
            COMBO_CATEGORIES.ValueMember = "ID_CAT";
            try
            {
            ...
            if (fq.addTheProduct)// test if the user cancel the add product to order | addTheProduct variable is declared in the quantity form
            {
                ...
                if (DGV_PRODUCTS_IN_ORDER.Rows.Count - 1 > 0)
                {
                    ...
                    if(isExist)
                    {
                        ...
                        int qty2 = Convert.ToInt32(table.Rows[0]["Qte_stock"].ToString());
                        if (pqty > qty2)
                        {
                            MessageBox.Show("Quantité non disponible", "Quantité", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            return;
        ...
        private void BTN_INSERT_ORDER_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Aucun client sélectionné", " Erreur client", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                ...
                COMBO_CATEGORIES_SelectedIndexChanged(null, null);
                MessageBox.Show("Commande Insérée avec Détails");
