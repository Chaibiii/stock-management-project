...
namespace Csharp_Project
{
    public partial class FORM_MANAGE_PRODUCT : Form
    {
        ...
        {
            if (MessageBox.Show("Voulez-vous vraiment supprimer ce produit", "Supprimer le produit", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                product.deleteProducts(Convert.ToInt32(DGV_PRODUCTS.CurrentRow.Cells[0].Value));
                DGV_PRODUCTS.DataSource = product.getProducts();
                MessageBox.Show("Produit supprimé avec succès", "Supprimer le produit");
            }
        }
