...
namespace Csharp_Project
{
    public partial class FORM_MANAGE_USER : Form
    {
        ...
        private void BTN_DELETE_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Voulez-vous supprimer cet utilisateur?", "Supprimer l'utilisateur", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    user.deleteUser(Convert.ToInt32(TB_ID.Text));
                    MessageBox.Show("L'utilisateur a été supprimé avec succès", "L'utilisateur supprimé", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    DGV_USERS.DataSource = user.getUsers();
                }
            }
            else
            {
                MessageBox.Show("Sélectionnez l'utilisateur que vous souhaitez supprimer");
            }
        ...
        private void BTN_UPDATE_Click(object sender, EventArgs e)
        {
            if (TB_ID.Text != string.Empty)
            {
                ...
                MessageBox.Show("L'utilisateur a été modifier avec succès", "Update User Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                DGV_USERS.DataSource = user.getUsers();
            }

            else
            {
                MessageBox.Show("Sélectionnez l'utilisateur que vous souhaitez modifier");
            }
        }
